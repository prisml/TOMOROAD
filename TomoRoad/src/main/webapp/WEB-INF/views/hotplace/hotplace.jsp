...
<div id="termsranking">
	<table id="termsrankingtable"></table>
</div>
...
<script>
	...
	$(document).ready(
					function() {
						$
								.ajax({
									type : "GET",
									url : "${pageContext.request.contextPath}/noauth_rankingLocation.do", //랭킹 지역들 가져옴
						...
						$
								.ajax({
									type : "GET",
									url : "${pageContext.request.contextPath}/noauth_rankingInfo.do", //랭킹 지역들의 인포를 가져옴
									success : function(data) {
										var ranking = "";
										for (var i = 0; i < data.length; i++) {
										...
										$("#termsrankingtable").html(ranking); //지도밑에 검색순위 텍스트로
									}
								});
					});
	...
	$(document).on("click","#searchRankingLocation",function() {
				...
				for (var i = 0; i < locationList.length; i++) {
					...
					infoList[i] = '<div id="content">'
								    ...
								    + '<h1 id="firstHeading" class="firstHeading">' + rankingInfoList[i].name
								    + '</h1>' + '<div id="bodyContent">' + '<p>' + rankingInfoList[i].detail + '</p>' + '</div>'
								  + '</div>';
	...
	function initMap() {
		...
		map = new google.maps.Map(document.getElementById('map'), {
			center : {
				lat : 37.554925,
				lng : 126.970831
			}, //사용자가 위치정보 전송을 원하지않을때는 이 좌표 (유스페이스)로 이동함.
			zoom : 7
		});
	...
	function handleLocationError(browserHasGeolocation, infoWindow, pos) {
		infoWindow.setPosition(pos);
		infoWindow
				.setContent(browserHasGeolocation ? 'Error: 현재위치 불러오기에 실패했습니다. 현재위치 정보 받아오기에 동의하지않으신 경우, 현재위치관련 서비스를  제공받을 수 없습니다.'
...
<br>
내 현재위치가 궁금하다면 ?
<input type="button" value="현재위치로" id="clientLocation">
